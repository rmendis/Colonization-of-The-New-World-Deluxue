...
function ViewOverall() 
	...
	local aPointsForVictory = {2250, 2500, 2750, 3000, 3250, 3500, 3750, 4000};
	 
	local szOutbackText1 = Locale.Lookup("LOC_NEW_WORLD_SCENARIO_DESCRIPTION");
	Controls.OutbackInfo1:SetText(szOutbackText1);
	
	local szHandicapText;
	szHandicapText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_DIFFICULTY", GameInfo.Difficulties[7].Name); 
	Controls.DeityInfo:SetText(szHandicapText); 
	szHandicapText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_DIFFICULTY", GameInfo.Difficulties[6].Name);
    Controls.ImmortalInfo:SetText(szHandicapText);
	szHandicapText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_DIFFICULTY", GameInfo.Difficulties[5].Name);
    Controls.EmperorInfo:SetText(szHandicapText);  
	szHandicapText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_DIFFICULTY", GameInfo.Difficulties[4].Name);
    Controls.KingInfo:SetText(szHandicapText); 
	szHandicapText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_DIFFICULTY", GameInfo.Difficulties[3].Name);
    Controls.PrinceInfo:SetText(szHandicapText); 
	szHandicapText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_DIFFICULTY", GameInfo.Difficulties[2].Name);
    Controls.WarlordInfo:SetText(szHandicapText); 
	szHandicapText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_DIFFICULTY", GameInfo.Difficulties[1].Name);
    Controls.ChieftainInfo:SetText(szHandicapText); 
	szHandicapText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_DIFFICULTY", GameInfo.Difficulties[0].Name);
    Controls.SettlerInfo:SetText(szHandicapText); 

	local szScoreText;
	szScoreText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_SINGLE_PLAYER_SCORE", aPointsForVictory[8]); 
	Controls.DeityScore:SetText(szScoreText); 
	szScoreText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_SINGLE_PLAYER_SCORE", aPointsForVictory[7]);
    Controls.ImmortalScore:SetText(szScoreText);
	szScoreText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_SINGLE_PLAYER_SCORE", aPointsForVictory[6]);
    Controls.EmperorScore:SetText(szScoreText);  
	szScoreText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_SINGLE_PLAYER_SCORE", aPointsForVictory[5]);
    Controls.KingScore:SetText(szScoreText); 
	szScoreText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_SINGLE_PLAYER_SCORE", aPointsForVictory[4]);
    Controls.PrinceScore:SetText(szScoreText); 
	szScoreText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_SINGLE_PLAYER_SCORE", aPointsForVictory[3]);
    Controls.WarlordScore:SetText(szScoreText); 
	szScoreText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_SINGLE_PLAYER_SCORE", aPointsForVictory[2]);
    Controls.ChieftainScore:SetText(szScoreText); 
	szScoreText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_SINGLE_PLAYER_SCORE", aPointsForVictory[1]);
    Controls.SettlerScore:SetText(szScoreText); 
	...
	local szDifficultyNameString = Locale.Lookup(GameInfo.Difficulties[gameDifficultyTypeID].Name);
	local szDifficultyText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_CURRENT_GAME_DIFFICULTY", szDifficultyNameString);
	Controls.DifficultyInfo:SetText(szDifficultyText); 

	szVictoryText = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_SINGLE_PLAYER_SCORE", aPointsForVictory[gameDifficultyIndex + 1]);
	Controls.VictoryInfo:SetText(szVictoryText);

	local szOutbackText2 = Locale.Lookup("LOC_SCENARIO_NEW_WORLD_TITLE") .. "[NEWLINE][NEWLINE]" .. Locale.Lookup("LOC_SCENARIO_NEW_WORLD_EFFECTS");
	Controls.OutbackInfo2:SetText(szOutbackText2); 

	if GameConfiguration.IsAnyMultiplayer() then
		Controls.SinglePlayerInfo:SetHide(true);
		Controls.OutbackInfo2:SetOffsetY(290);
	end
end
	...
	for teamID, team in pairs(Teams) do
		if teamID >= 0 then
			...
			table.insert(teamData, {
				-- Team Data
				TeamID = teamID,
				TeamScore = Game.GetVictoryProgressForTeam(victoryType, teamID),
...
function GatherScoreData()
	...
	for teamID, team in pairs(Teams) do
		if teamID >= 0 then
			...
			for i, playerID in ipairs(team) do
				local pPlayer:table = Players[playerID];
				if IsAliveAndMajor(playerID) and pPlayer:IsHuman() then
		...
		if teamID >= 0 then
			...
			for i, playerID in ipairs(team) do
				if IsAliveAndMajor(playerID) then
					...
					local playerData:table = { 
						PlayerID = playerID, 
						NumRequiredTourists = pPlayer:GetCulture():GetStaycationers(),
...
function PopulateCultureInstance(instance:table, playerData:table)
	...
	local civName:string, civIcon:string = GetCivNameAndIcon(playerData.PlayerID, true);
	...
	if(textureSheet == nil or textureSheet == "") then
		...
		instance.CivIcon:SetTexture(textureOffsetX, textureOffsetY, textureSheet);
		SetLeaderTooltip(instance, playerData.PlayerID, details);
	end
...
function PopulateDominationInstance(instance:table, playerData:table)
	local pPlayer:table = Players[playerData.PlayerID];
	local backColor, frontColor = UI.GetPlayerColors(playerData.PlayerID);
	local civName:string, civIcon:string = GetCivNameAndIcon(playerData.PlayerID, true);
